...
/*   Updated: 2024/11/09 15:58:24 by pshamkha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move(t_game *g, int direction)
{
    ...
    if ((int)(g->ray.pos.y + move_dir.y) >= 0 && (int)(g->ray.pos.y + move_dir.y) < g->map_height
        && g->map[(int)(g->ray.pos.y + move_dir.y)][(int)(g->ray.pos.x)] == '0')
        g->ray.pos.y += move_dir.y;
    if ((int)(g->ray.pos.x + move_dir.x) >= 0 && (int)(g->ray.pos.x + move_dir.x) < g->map_width
        && g->map[(int)(g->ray.pos.y)][(int)(g->ray.pos.x + move_dir.x)] == '0')
        g->ray.pos.x += move_dir.x;
}


void	rotate(t_game *g, int direction, double angle)
{
	double old_dir_x;
	double old_plane_x;
	...
	if (direction == EA)
	{
		...
		g->ray.dir.x = g->ray.dir.x * cos(-angle) - g->ray.dir.y * sin(-angle);
    	g->ray.dir.y = old_dir_x * sin(-angle) + g->ray.dir.y * cos(-angle);
    	old_plane_x = g->ray.plane.x;
    	g->ray.plane.x = g->ray.plane.x * cos(-angle) - g->ray.plane.y * sin(-angle);
    	g->ray.plane.y = old_plane_x * sin(-angle) + g->ray.plane.y * cos(-angle);
	}
	else if (direction == WE)
	{
		old_dir_x = g->ray.dir.x;
